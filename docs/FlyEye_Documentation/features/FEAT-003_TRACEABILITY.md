...
Current status superseding the documentation-only baseline: FEAT-003 was merged
through PR #7 at `00dcfd2239de45b33890bc805ff27a667defabd1`; required CI
run `30752548762` passed. The parent feature is `Merged; hosted validation
pending`. A bounded follow-up hardening slice on
`feat/FEAT-003-hosted-staging-hardening` removes avoidable repository-level
risk before any hosted synthetic validation decision. It adds a disabled exact
staging manifest, strict fail-closed runtime profiles, dormant constrained
staging provenance, and versioned limiter evidence without adding a hosted
issuer, provider target, credential, deployment, or FEAT-004 behavior.

The parent feature passed the complete local synthetic application/database
matrix, all three runtime fixtures, repository and history secret scans, and
exact zero-residue cleanup on 2026-08-02.
...
| FEAT-003-DESIGN-AMD-05 | Which repository controls must precede any hosted synthetic validation decision? | Keep staging disabled with no target; require an exact version-controlled staging manifest; distinguish dormant `staging_fixture` provenance from `local_fixture`; reject remote, staging, production, ambiguous, or non-synthetic runtime configuration; record the exact limiter policy version; require distinct key roles; and add no executable hosted issuer until the exact provider/project is separately approved and statically bound. | Complete local matrix, secret/history scans, scope inspection, and fresh separate-agent review are required before publishing this hardening slice. Hosted provider, project, cost, credential, retention, cleanup, execution, deployment, real-data, production, and FEAT-004 gates remain closed. | Local evidence and corrected independent review passed 2026-08-05; publication pending |
...
| FEAT-003-23 | Hosted synthetic validation must not accidentally target production or reuse local-only provenance | Security Requirements sections 4, 6, 9, and 11; ADR-0005 | Disabled exact staging manifest; fail-closed local-only runtime; dormant distinct staging provenance; versioned limiter policy; no executable hosted issuer | Manifest validator; shared Edge runtime configuration; additive constrained audit/grant provenance; limiter policy-version column and response | Reject before privileged client work; exact loopback/origin/data/policy/key-role checks; all remote/staging/production runtime profiles unsupported; browser/application issuance absent; human attribution cannot be fabricated | DESIGN-AMD-05; FEAT-003-HS-AC-01 through AC-08; focused manifest/config/handler tests; SQL provenance/policy tests; complete local matrix and scans | Local application/database/runtime/scans and corrected independent review passed 2026-08-05; publication pending | Bounded follow-up authorized 2026-08-05; exact hosted activation remains a separate decision |
...
| Implementation | Separate explicit local synthetic authorization and implementation branch | Parent implementation merged through PR #7 at `00dcfd2239de45b33890bc805ff27a667defabd1`; repository risk hardening is locally implemented on `feat/FEAT-003-hosted-staging-hardening` |
...
| Local verification | Full planned matrix, exact implementation diff, minimized evidence, known limitations | Parent completed 2026-08-02; hardening rerun completed 2026-08-05 with 143 application tests, 107 SQL/RLS tests, all three runtime fixtures, zero known dependency advisories, repository/history secret scans, zero-residue cleanup, and current generated types |
| Local technical publication gate | Local verification plus corrected separate-agent review | Parent completed 2026-08-02; hardening findings corrected and final re-review passed 2026-08-05 |
| Git staging, commit, and publication | Standing bounded delivery envelope after local verification and separate-agent review | Parent published and merged through PR #7; hardening slice publication pending; merge remains human-only |
| Hosted validation | Separate exact synthetic hosted environment/provider/cost/privacy/credential/retention/cleanup authorization | Not granted; disabled manifest has no provider target and no executable hosted issuer exists |
