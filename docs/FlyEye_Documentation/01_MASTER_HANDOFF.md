...
1. Obtain one explicit bounded override of FEAT-003's earlier no-publication instruction. After it is granted, apply the autonomous-delivery directive: stage only the bounded feature/governance files, create the reviewed commit, push the feature branch, open a review-ready pull request, and follow CI through green. No form or specialist review is required for this transition; stop for the Founder/Product Owner merge decision.
2. Keep hosted validation, deployment, real-data use, and production approval separate. Do not rerun an unchanged final matrix or review target.
3. Do not begin FEAT-004 Member Invitations, FEAT-005 User Management and Basic Profiles, or FEAT-006 Role Assignment until its predecessor is merged and the next bounded outcome is requested. Each request then carries its complete delivery through a green review-ready pull request.
