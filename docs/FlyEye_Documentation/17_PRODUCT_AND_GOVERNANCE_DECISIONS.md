...
- One person may hold separate memberships in multiple organizations. Selecting one organization never widens access to another.
...
- MFA is mandatory for every user before real-data pilot or production access. TOTP is the initial method; SMS is not the primary method.
