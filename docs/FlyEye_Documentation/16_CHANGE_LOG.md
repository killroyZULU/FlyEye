...
- Implemented FEAT-004 Member Invitations for local synthetic verification: protected create/list/resend/revoke/prepare/accept commands, complete-scope idempotency, deny-by-default tenant data, atomic membership/role/audit acceptance, new and existing Auth recipient paths, one-hour expiry, server-resolved rate limits, responsive confirmation UI, and zero-residue runtime coverage. Hosted email, real data, deployment, production, merge, and qualified review remain separate gates.
