...
- Recorded the Founder/Product Owner’s FEAT-002 synthetic-local planning decisions for recovery eligibility, password controls, credential expiry and verification, session revocation, exact loopback redirects, Mailpit notifications, provider-neutral CAPTCHA, cooldown, Auth-log evidence, environment limits, and deferred recovery scope. Separated pre-implementation decisions, runtime acceptance evidence, and real-data/production gates; added the environment configuration matrix; normalized all 20 planned test IDs in traceability; and recorded that technical, independent security/privacy, and accessibility reviewers are not assigned. This is documentation and product-direction approval only: implementation remains unauthorized, and nothing was implemented, configured, deployed, or production-approved.
