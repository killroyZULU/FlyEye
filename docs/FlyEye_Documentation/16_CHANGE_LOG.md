...
- Adopted [ADR-0006](adr/ADR-0006-SINGLE-SCHOOL-ISOLATED-DEPLOYMENTS.md): each flight school receives a separate application and Supabase/cloud/data deployment from one versioned product. The bounded FIX-006 revision removes school selection, enforces one school and one membership per user per deployment, retains internal organization/RLS defenses, permits non-privileged Student AAL1, and keeps Instructor/Admin and authority-bearing access at TOTP/AAL2.
